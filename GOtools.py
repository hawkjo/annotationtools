...
def slim_id_set_given_go_id_dict(GO_dict,
                                 slim_terms_set,
    ...
    def set_slim_info(go_id):
        ...
        if (slim_info_given_go_id[go_id]['slims_to_set']
                and slim_info_given_go_id[go_id]['dist'] is not None):
            ...
            parent_list = get_go_id_parents(GO_dict,
                                            go_id,
                                            include_part_of,
            ...
            slim_info_given_go_id[go_id]['dist'] = min(
                slim_info_given_go_id[parent]['dist'] + 1 for parent in parent_list)
            for parent in parent_list:
...
def get_go_id_parents(GO_dict, go_id, include_part_of=True, include_all_relationships=False):
    ...
    parent_list = []
    base_terms = ['biological_process', 'molecular_function', 'cellular_component']
    if 'is_a' not in GO_dict[go_id]:
        if 'is_obsolete' not in GO_dict[go_id] and GO_dict[go_id]['name'] not in base_terms:
...
def traverse_graph_looking_for_target(GO_dict,
                                      current_id,
                                      target_id,
                                      path_before_current_id=[],
                                      include_part_of=True,
                                      include_all_relationships=False):
    """
    ...
    for parent in get_go_id_parents(GO_dict, current_id, include_part_of,
        ...
        if parent == target_id:
            ...
            paths.extend(traverse_graph_looking_for_target(GO_dict, parent, target_id,
                                                           path_before_current_id + [current_id],
                                                           include_part_of,
                                                           include_all_relationships))
    return paths


def find_paths_between(GO_dict, go_id1, go_id2,
                       include_part_of=True,
                       include_all_relationships=False):
    """
    ...
    paths = []
    paths.extend(
        traverse_graph_looking_for_target(GO_dict, go_id1, go_id2,
                                          include_part_of=include_part_of,
                                          include_all_relationships=include_all_relationships))
    paths.extend(
        traverse_graph_looking_for_target(GO_dict, go_id2, go_id1,
                                          include_part_of=include_part_of,
                                          include_all_relationships=include_all_relationships))
    return paths
...
def map2slim(GO_filename, slim_filename, gaf_filename, out_filename,
    ...
    slim_id_set_given_go_id = slim_id_set_given_go_id_dict(GO_dict,
                                                           slim_terms_set,
                                                           include_part_of,
                                                           include_all_relationships)
    ...
    with open(out_filename, 'w') as outfile:
        ...
        for line in sorted(lines_to_write):
            # First two columns are db and db id, so regular string sorting results in desired
            # output.
            outfile.write(line)
...
if __name__ == "__main__":
    ...
    print 'Terms without parents:',
    print sum(1 for go_id in GO_dict.keys() if 'is_a' not in GO_dict[go_id])
